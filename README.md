...
- 기준 Unity: Unity 2022.3 LTS 우선, Unity 6는 별도 검증
- 1차 인터페이스: `/cmd_vel`, `/odom`, `/tf`, `/scan`, `/clock`
- 현재 구현: Jazzy rosbridge 컨테이너, ROS 2 왕복 확인 노드, Unity 차동구동 제어 골격
- 검증 완료: ARM64 Jazzy 이미지 빌드, rosbridge 2.7.0 기동, heartbeat와 `/cmd_vel` ROS 측 왕복
- 아직 검증하지 않은 항목: Unity Editor 실제 접속, 센서 처리량, SLAM/Nav2 종단 간 실행
...
Unity 설정은 [02_개발환경_구축](docs/book/02_개발환경_구축.md)부터 진행합니다.
